<template>
  <div
    class="card card-consulta-processual mb-3"
    style="background-color: #f8ff99"
  >
    <div class="card-header">
      <strong>{{ titulo }}</strong>
      <span
        class="close-button d-print-none icone-em-linha"
        aria-hidden="true"
        @click="esconderNotas()"
      >
      ...
      <span
        v-if="state === 'changed'"
        class="pull-right chars-left icone-em-linha"
        >{{ limit - count }}/{{ limit }} restantes</span
      >
      <span
        v-if="errormsg === undefined &amp;&amp; (state === undefined || state === 'processing')"
        class="pull-right processing icone-em-linha"
      >
    ...
    <div class="card-body">
      ...
      <textarea
        :disabled="state === undefined || errormsg !== undefined"
        ref="textarea"
        v-model="textoAtual"
        @input="notaAlterada"
      ></textarea>
...
export default {
  ...
  computed: {
    count: function() {
      return this.textoAtual !== undefined
        ? UtilsBL.getUTF8Length(this.textoAtual)
        : 0;
    }
  },
  ...
  methods: {
    carregarNota: function() {
      ...
      this.$http
        .get("processo/" + this.processo + "/nota?sistema=" + this.sistema)
        .then(
          response => {
    ...
    notaAlterada: function() {
      if (this.textoAtual !== undefined && this.textoAtual.trim() === "")
        this.textoAtual = undefined;
    ...
    postNota: function(texto) {
      ...
      this.$http
        .post("processo/" + this.processo + "/nota?sistema=" + this.sistema, {
          texto: texto,
          pessoal: this.pessoal
        })
        .then(
