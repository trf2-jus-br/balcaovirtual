...
export default {
  ...
  methods: {
    carregarLista: function () {
      var i
      this.$http.get('aviso-confirmado/listar?datainicial=' + this.dataInicial + '&datafinal=' + this.dataFinal +
        '&confirmacao=' + this.porConfirmacao + '&omissao=' + this.porOmissao + '&grupo=' + this.doGrupo, { block: true }).then(response => {
          ...
          if (response.data.list) {
            ...
            for (i = 0; i < response.data.list.length; i++) {
              ...
              aviso.dataconfirmacaoFormatada = UtilsBL.formatDDMMYYYYHHMM(aviso.dataconfirmacao)
