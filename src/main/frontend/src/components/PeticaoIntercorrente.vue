          ...
          <table class="table table-peticao">
            ...
            <tbody>
              <tr v-for="f in arquivos" :class="{odd: f.odd}">
                ...
                <td v-show="!f.anexo" :rowspan="f.rowspan">
                  <v-selectpage  v-if="editando && avisos[f.processo] !== undefined && avisos[f.processo].length > 1" :data="avisos[f.processo]" v-model="f.encerraprazos" key-field="idaviso" show-field="destinatariodocumento" :tb-columns="columns" :multiple="true" title="Selecione um ou mais" :pagination="false" placeholder="[Selecione]"></v-selectpage>
