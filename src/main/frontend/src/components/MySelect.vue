<template>
  <div>
    ...
    <select v-if="edit" :id="name" class="form-control" :disabled="disabled" v-bind:value="value" v-on:input="$emit('input', $event.target.value)" v-on:change="$emit('change')" :name="name" :class="{ 'is-invalid': errors.length > 0 }" v-bind="$attrs">
