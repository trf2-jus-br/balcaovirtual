  ...
  <div class="container-fluid content">
    <div class="row">
      ...
      <div class="col col-sm-12" v-if="tentouBaixar">
        <p class="alert alert-danger">
          ...
          <a href="https://chrome.google.com/webstore/detail/trf2-download-manager/komegelldppbjndifhabfpjpddjaocfa"> <u>Chrome Web Store</u> </a>. Também verifique na Lista de
          Extensões (Menu/Mais ferramentas/Extensões) se 'TRF2 Download Manager' está presente e ativa.
        </p>
      </div>
    </div>
    ...
    <div class="row" v-if="filtrados.length == 0">
      <div class="col col-sm-12">
        <p class="alert alert-warning"><strong>Atenção!</strong> Nenhum processo na lista.</p>
      </div>
    </div>
            ...
            <tbody>
              <tr v-for="p in filtrados" :key="p.numero">
                <td style="text-align: center">
                  <input type="checkbox" v-model="p.checked" :disabled="p.disabled" />
                </td>
                <td>
                ...
                <td class="status-td">
                  ...
                  <span v-if="p.errormsg" :class="{ red: true }">{{ p.errormsg }} </span>
                </td>
...
export default {
  ...
  methods: {
    ...
    obterJwt: function(processos, i) {
      ...
      this.$http.get("processo/" + processos[i].numero + "/pdf?sistema=" + processos[i].sistema).then(
        response => {
    ...
    baixando: function() {
      ...
      for (var i = 0; i < processos.length; i++) {
        if (processos[i].jwt && ["building", "ready", "set", "go", "in_progress"].indexOf(processos[i].state) >= 0) c++;
      }
      return c;
    },
    ...
    atualizarStatusDeConstrucaoDeProcesso: function(processo) {
      this.$http.get("status/" + processo.uuid).then(
        ...
        error => {
          ...
          processo.errormsg = error.data.errormsg || "Erro obtendo informações de status";
        }
      );
    },
    ...
    baixar: function(processo) {
      ...
      if (url.substring(0, 4) !== "http") {
        var host = window.location.protocol + "//" + window.location.hostname + (window.location.port && window.location.port !== "" ? ":" + window.location.port : "");
        url = host + url;
      }
      chrome.runtime.sendMessage(
        this.downloadExtensionId,
    ...
    sinalizar: function(p, sinais, lote) {
      ...
      this.$http.post("processo/" + p.numero + "/sinalizar", sinais, { block: !lote }).then(
        response => {
          var d = response.data;
