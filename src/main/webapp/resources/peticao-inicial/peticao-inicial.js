var appPI = angular.module('sample.peticao-inicial', [
...
appPI.config(function($stateProvider) {
	$stateProvider.state('peticao-inicial', {
		url : '/peticao-inicial',
		controller : 'PeticaoInicialCtrl',
		templateUrl : 'resources/peticao-inicial/peticao-inicial.html',
		data : {
			requiresLogin : true
		},
		resolve : {
...
appPI
		.controller(
				'PeticaoInicialCtrl',
				function PeticaoInicialCtrl($scope, $http, store, jwtHelper,
						Upload, $timeout, ModalService) {
					const
					...
					$scope.adicionarArquivo = function() {
						$timeout(function() {
							angular.element('#ngf').triggerHandler('click');
						});
					};

					$scope.adicionarParte = function() {
						if ($scope.partes.length == 0)
							$scope.partes.push({});
						else {
							var last = $scope.partes[$scope.partes.length - 1];
							$scope.partes.push({
								polo : last.polo,
								tipopessoa : last.tipopessoa
							});
						}
					};
