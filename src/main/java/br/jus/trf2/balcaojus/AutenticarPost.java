...
import br.jus.trf2.balcaojus.IBalcaojus.IAutenticarPost;
...
public class AutenticarPost implements IAutenticarPost {
	...
	public static String getAuthorizationHeader() throws SwaggerAuthorizationException {
		String authorization = BalcaojusServlet.getHttpServletRequest().getHeader("Authorization");

		if (authorization == null) {
			Cookie[] cookies = SwaggerServlet.getHttpServletRequest().getCookies();
			if (cookies != null) {
				for (Cookie c : cookies) {
					if (BalcaojusServlet.JWT_AUTH_COOKIE_NAME.equals(c.getName()))
	...
	private static String jwt(String origin, String username, String cpf, String name, String email, String users)
		...
		final long exp = iat + BalcaojusServlet.JWT_AUTH_COOKIE_TIME_TO_EXPIRE_IN_S; // token expires in 18 hours
	...
	public static Cookie buildCookie(String tokenNew) {
		Cookie cookie = new Cookie(BalcaojusServlet.JWT_AUTH_COOKIE_NAME, tokenNew);
		cookie.setPath("/");
		if (BalcaojusServlet.JWT_AUTH_COOKIE_DOMAIN != null)
			cookie.setDomain(BalcaojusServlet.JWT_AUTH_COOKIE_DOMAIN);

		cookie.setMaxAge(BalcaojusServlet.JWT_AUTH_COOKIE_TIME_TO_EXPIRE_IN_S);
	...
	public static Cookie buildEraseCookie() {
		Cookie cookie = new Cookie(BalcaojusServlet.JWT_AUTH_COOKIE_NAME, "");
		cookie.setPath("/");
		if (BalcaojusServlet.JWT_AUTH_COOKIE_DOMAIN != null)
			cookie.setDomain(BalcaojusServlet.JWT_AUTH_COOKIE_DOMAIN);
